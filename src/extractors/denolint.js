...
function pretty(lines) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (let j = i + 2; j < lines.length && !HEAD_RE.test(lines[j]); j++) {
      ...
      if (source && +source[1] === +at[2] && stmt === undefined) stmt = source[2].trim();
...
function compact(lines) {
  ...
  for (const line of lines) {
    ...
    if (!m || !SCRIPT.test(m[1])) continue;
    out.push({ file: unfile(m[1]), line: +m[2], col: +m[3], title: m[5], code: m[5], severity: "error", message: m[4] });
  }
  return out;
}
...
function json(s) {
  ...
  const doc = findJsonDocument(s, REPORT);
...
function findings(s) {
  ...
  const out = [];
  const seen = new Set();
  for (const f of [...pretty(lines), ...compact(lines), ...json(s)]) {
...
export default {
  ...
  extract(s) {
    const failures = findings(s).map(({ hint, ...f }) => ({
...
export const denoFmt = {
  ...
  extract(s) {
    ...
    for (const line of s.split("\n")) {
      const m = line.match(FMT_FILE_RE);
      if (!m) continue;
      failures.push({
