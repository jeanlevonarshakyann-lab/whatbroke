...
// for the location.
const FAILED_RE = /^failed\s+(\S+)\s+\((\d+(?:\.\d+)?)\s*m?s\)\s*$/;
const AT_RE = /^\s*at\s+(.+?)\s+in\s+(.+?):(?:line\s+)?(\d+)\s*$/;
  ...
  extract(s) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      for (let j = i + 1; j < lines.length && !FAILED_RE.test(lines[j]); j++) {
        ...
        if (/^\s*from\s+\S+\.dll/.test(lines[j])) { repeated = true; continue; }
