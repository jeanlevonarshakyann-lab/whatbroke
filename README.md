...
| **go refusing a flag** | `flag provided but not defined: -x` comes from Go's flag package, which every Go program uses, so the line alone says nothing about whose flag it was — go's own `usage: go <verb>` underneath is what does |
