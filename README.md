...
| **Docker / BuildKit** | the Dockerfile line it marked with `>>>`, and the step's own error rather than the `failed to build: failed to solve:` restatement of it at the end. Not `process "…" did not complete successfully` — that is docker relaying an inner command's exit status, and the tool that actually failed says it better. When the command inside is not a tool anything here reads — `/bin/sh: nosuchcommand: not found` — nothing else can explain the log, and docker is asked again: the step's own last word as the message, on the Dockerfile line that was running |
